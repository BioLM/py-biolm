...
class BiolmaiRequestHeaderProvider(RequestHeaderProvider):
    ...
    def _get_valid_token(self) -> Optional[str]:
        ...
        try:
            ...
            expires_at = creds.get(
                "expires_at"
            )  # Optional: if biolmai stores expiration
            ...
            if refresh_token and expires_at and time.time() >= (expires_at - 60):
                ...
                if new_token:
                    ...
                    if "expires_at" in creds:
                        ...
                        creds["expires_at"] = time.time() + creds.get(
                            "expires_in", 3600
                        )
    ...
    def _refresh_token(self, refresh_token: str, creds: dict) -> Optional[str]:
        ...
        try:
            ...
            OAUTH_TOKEN_URL = (
                os.environ.get("OAUTH_TOKEN_URL") or f"{base_domain}/o/token/"
            )

        token_url = creds.get("token_url") or OAUTH_TOKEN_URL

        client_id = (
            creds.get("client_id")
            or BIOLMAI_PUBLIC_CLIENT_ID
            or os.environ.get("BIOLMAI_OAUTH_CLIENT_ID")
        )

        # Check both CLIENT_SECRET and BIOLMAI_OAUTH_CLIENT_SECRET for compatibility
        client_secret = (
            creds.get("client_secret")
            or BIOLMAI_OAUTH_CLIENT_SECRET
            or os.environ.get("BIOLMAI_OAUTH_CLIENT_SECRET")
            or os.environ.get("CLIENT_SECRET")
        )
