...
class ExampleGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None):
        ...
        self._http_client = HttpClient(
            self.base_url.rstrip("/") + "/", self._headers, DEFAULT_TIMEOUT
        )
    ...
    async def get_model_schema(
        self, model_name: str, action: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        client = BioLMApiClient(
            model_name, api_key=self.api_key, base_url=f"{self.base_url}/api/v3"
        )
        try:
...
result = model.{action}(items=[{{"{input_type}": "{sample_input}"}}])"""
        ...
        if schema:
            ...
            if params_schema and params_schema.get("properties"):
                ...
                for param_name, param_info in list(
                    params_schema.get("properties", {}).items()
                )[:1]:
...
result = model.{action}(items=[{{"{input_type}": "{sample_input}"}}], params={params_str})"""
    ...
    def _generate_markdown_example(
        ...
        python_code = self._generate_python_example(
            model_name, action, input_type, schema
        )
...
{indented_code}"""
    ...
    async def generate_example(
        ...
        if action is None:
            ...
            model_info = next(
                (
                    ...
                    for m in models
                    if (
                        m.get("model_slug") == model_name
                        or m.get("slug") == model_name
                        or m.get("model_name") == model_name
                        or m.get("name") == model_name
                    )
                ),
                None,
            )
        ...
        return await self._generate_single_example(
            model_name, action, format, input_type, schema
        )
...
def get_model_details(
    ...
    generator = ExampleGeneratorSync(api_key=api_key, base_url=base_url)
    try:
        return generator.fetch_model_details(
            model_slug, code_examples, exclude_docs_html
        )
    finally:
        generator.shutdown()
