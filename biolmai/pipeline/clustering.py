...
class SequenceClusterer:
    ...
    def cluster(
        ...
        if n > VERY_LARGE_DATASET_THRESHOLD and not self.mini_batch and self.method == 'kmeans':
            warnings.warn(
        ...
        if self.similarity_metric == 'embedding':
            if embeddings is None:
                raise ValueError("embeddings required when similarity_metric='embedding'")
    ...
    def _compute_embedding_distances(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        if not SCIPY_AVAILABLE:
            raise ImportError("scipy is required for embedding distances. Install with: pip install scipy")
    ...
    def _cluster_kmeans(self, distance_matrix: np.ndarray, n_samples: int) -> np.ndarray:
    ...
    def _cluster_hierarchical(self, distance_matrix: np.ndarray) -> np.ndarray:
        ...
        cluster_ids = fcluster(Z, self.n_clusters, criterion='maxclust') - 1  # 0-indexed
...
class DiversityAnalyzer:
    ...
    def shannon_entropy(sequences: List[str], normalize: bool = True) -> float:
        ...
        for pos in range(max_len):
            ...
            entropy = -np.sum(probs * np.log2(probs + 1e-10))  # Add epsilon for stability
    ...
    @staticmethod
    def motif_diversity(sequences: List[str], k: int = 3) -> Dict[str, Union[int, float]]:
        """
    ...
    def compute_all_metrics(
        ...
        metrics = {
            ...
            'pairwise_distances': cls.pairwise_distance_stats(sequences, max_sample=max_sample),
