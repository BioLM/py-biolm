...
class MLMRemasker:
        ...
        if self.config.mask_strategy == 'random':
            ...
            while len(positions) < num_to_mask:
                ...
                for i in range(start, min(start + block_size, seq_len)):
                    if i not in conserved and i not in positions and i in available_positions:
                        positions.append(i)
    ...
    async def predict_masked_positions(
        ...
        Args:
            ...
            mask_positions: Positions that are masked (original positions in unmasked sequence)
        ...
        if self.api_client is None:
            ...
            # Replace all mask tokens with random amino acids
            predicted_seq = masked_sequence
            for pos in mask_positions:
                replacement = self.random.choice(amino_acids)
                # Replace first occurrence of mask token
                predicted_seq = predicted_seq.replace(self.config.mask_token, replacement, 1)
    ...
    async def generate_variant(
        ...
        num_mutations = sum(
            1 for i, (a, b) in enumerate(zip(parent_sequence, current_sequence))
            if a != b
        )
        
        mutation_rate = num_mutations / len(parent_sequence) if len(parent_sequence) > 0 else 0
        ...
        metadata = {
            ...
            'avg_confidence': np.mean(list(all_confidences.values())) if all_confidences else 0.0,
