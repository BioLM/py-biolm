...
async def test_prediction_stage_streaming(datastore):
    ...
    # Mock input data
    df = pd.DataFrame({
        'sequence': ['ACDEFG', 'ACDEFGH', 'ACDEFGHI']
    })
    
    # Check that process_streaming method exists
    assert hasattr(stage, 'process_streaming'), "PredictionStage should have process_streaming"
...
def test_filter_stage_knows_if_streamable(datastore):
    ...
    assert not stage1.requires_complete_data, "ThresholdFilter stage should be streamable"
...
async def test_pipeline_identifies_streaming_opportunity():
    ...
    from biolmai.pipeline.data import PredictionStage, FilterStage
    from biolmai.pipeline.filters import ThresholdFilter, RankingFilter
...
def test_all_base_filters_categorized():
    ...
    for filter_cls in filter_classes:
        ...
        assert hasattr(filter_cls, 'requires_complete_data'), \
