...
class TestModelRun:
    ...
    def test_run_encode_with_fasta(
        self, mock_model_class, sample_fasta_file, mock_model
    ):
    ...
    @patch("biolmai.cli.Model")
    def test_run_with_params_file(
        self, mock_model_class, sample_fasta_file, tmp_path, mock_model
    ):
    ...
    @patch("biolmai.cli.Model")
    def test_run_different_actions(
        self, mock_model_class, sample_fasta_file, mock_model
    ):
    ...
    def test_run_missing_input(self, mock_model_class, mock_model):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["model", "run", "esm2-8m", "encode"])
    ...
    def test_run_invalid_file(self, mock_model_class, mock_model):
        ...
        result = runner.invoke(
            cli,
            ["model", "run", "esm2-8m", "encode", "--input", "/nonexistent/file.fasta"],
        )
    ...
    def test_run_with_batch_size(
        self, mock_client_class, mock_model_class, sample_fasta_file, mock_model
    ):
        ...
        mock_client.schema = AsyncMock(
            return_value={"properties": {"items": {"maxItems": 50}}}
        )
    ...
    def test_run_with_progress(self, mock_model_class, sample_fasta_file, mock_model):
        ...
        mock_model.encode.return_value = [{"embedding": [0.1, 0.2]} for _ in range(10)]
        mock_model_class.return_value = mock_model
    ...
    @patch("biolmai.cli.Model")
    def test_run_with_type_override(
        self, mock_model_class, sample_csv_file, mock_model
    ):
    ...
    def test_run_api_error(self, mock_model_class, sample_fasta_file):
        ...
        runner = CliRunner()
        result = runner.invoke(
            cli, ["model", "run", "esm2-8m", "encode", "--input", sample_fasta_file]
        )
