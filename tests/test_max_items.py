...
async def test_get_max_batch_size_returns_value_from_schema():
    ...
    schema = {"properties": {"items": {"maxItems": expected_max_items}}}
...
async def test_schema_cache_is_used():
    model_name = "test-model"
    action = "encode"
    schema = {"properties": {"items": {"maxItems": 10}}}
    api_client = client_mod.BioLMApiClient(model_name)
...
async def test_schema_returns_none_on_http_error():
    ...
    with patch.object(
        api_client._http_client, "get", new=AsyncMock(side_effect=Exception("fail"))
    ):
...
async def test_batch_call_with_schema_uses_max_batch(monkeypatch):
    ...
    monkeypatch.setattr(
        api_client, "_get_max_batch_size", AsyncMock(return_value=max_batch)
    )
    # Patch _batch_call/batch_call to just return the items it receives
    monkeypatch.setattr(
        api_client, "call", AsyncMock(side_effect=lambda *a, **kw: a[1])
    )
