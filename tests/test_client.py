...
def test_mixed_sequence_batch_lol_items_first_invalid(model):
    items = [
        [{"sequence": "MENDELSEMYEFF::FEEFMLYRRTELSYYYUPPPPPU"}],
        [{"sequence": "MDNELE"}],
    ]
    result = model.predict(items=items)
    assert isinstance(result, list)
...
def test_single_predict_to_disk(tmp_path, model):
    ...
    model.predict(
        items=[{"sequence": "MDNELE"}], output="disk", file_path=str(file_path)
    )
...
def test_single_predict_to_disk_with_error(tmp_path, model):
    file_path = tmp_path / "out.jsonl"
    model.predict(
        items=[{"sequence": "MD::NELE"}], output="disk", file_path=str(file_path)
    )
    assert file_path.exists()
    lines = file_path.read_text().splitlines()
    assert len(lines) == 1
...
def test_batch_predict_to_disk_stop_on_error(tmp_path, model):
    ...
    file_path = tmp_path / "batch.jsonl"
    model.predict(
        items=items, output="disk", file_path=str(file_path), stop_on_error=True
    )
    assert file_path.exists()
    lines = file_path.read_text().splitlines()
    LOGGER.warning(lines)
...
def test_batch_predict_to_disk_continue_on_error(tmp_path, model):
    ...
    file_path = tmp_path / "batch.jsonl"
    model.predict(
        items=items, output="disk", file_path=str(file_path), stop_on_error=False
    )
    assert file_path.exists()
    lines = file_path.read_text().splitlines()
    LOGGER.warning(lines)
...
def test_batch_predict_to_disk_continue_on_error_multiple_per_batch(tmp_path, model):
    items = [
        [{"sequence": "MDNELE"}, {"sequence": "MDNELE"}],
        [{"sequence": "DN::A"}, {"sequence": "MDNELE"}],
        [{"sequence": "ISOTYPE"}, {"sequence": "ISOTYPE"}],
    ]
...
def test_invalid_input_items_type(model, monkeypatch):
    ...
    monkeypatch.setattr(
        model, "_batch_call_autoschema_or_manual", lambda *a, **kw: None
    )
    # Should raise TypeError because items is not a list/tuple
    with pytest.raises(
        TypeError, match="Parameter 'items' must be of type list, tuple"
    ):
...
def test_disk_output_skip_existing_file(tmp_path, model):
    ...
    result = model.predict(
        items=items, output="disk", file_path=str(file_path), overwrite=False
    )
...
def test_disk_output_overwrite_existing_file(tmp_path, model):
    ...
    result = model.predict(
        items=items, output="disk", file_path=str(file_path), overwrite=True
    )
