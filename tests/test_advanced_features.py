...
class TestPipelineResumability:
    ...
    def test_cached_predictions_are_reused(self, tmp_path, datastore):
        ...
        uncached = [seq for seq in sequences if not datastore.has_prediction(seq, 'tm', 'test_model')]
        assert len(uncached) == 1, f"Should have 1 uncached sequence, got {len(uncached)}"
...
class TestStreamingBehavior:
    ...
    async def test_streaming_yields_incrementally(self, datastore):
        ...
        async for chunk in stage.process_streaming(df, datastore):
            ...
            if chunks_received == 2:
                ...
                assert stage._api_client.call_count >= 2, "Should have made multiple API calls"
                print(f"  ✓ Streaming verified: Received chunk {chunks_received} while batches still in flight")
        
        # Verify we got multiple chunks
        assert chunks_received >= 3, f"Should receive multiple chunks, got {chunks_received}"
        ...
        if len(receive_times) >= 2:
            ...
            assert time_between_chunks < 1.5, f"Chunks should stream reasonably fast, got {time_between_chunks:.3f}s"
            print(f"  ✓ Timing verified: {time_between_chunks:.3f}s between first chunks")
    ...
    async def test_batching_waits_for_all(self, datastore):
        ...
        print(f"  ✓ Batching verified: Waited {elapsed:.3f}s for all {len(sequences)} sequences")
...
class TestErrorHandling:
    ...
    async def test_skip_on_error_marks_failures(self, datastore):
        ...
        for seq in sequences:
            ...
            assert matching.iloc[0]['value'] is None or pd.isna(matching.iloc[0]['value'])
            print(f"  ✓ {seq} marked as failed in cache")
        
        print(f"  ✓ skip_on_error successfully prevents pipeline failure")
