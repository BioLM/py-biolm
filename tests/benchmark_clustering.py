...
from biolmai.pipeline.clustering import analyze_diversity, cluster_sequences
...
def benchmark_clustering():
    ...
    for n in test_sizes:
        ...
        if n <= 10000:
            ...
            print("  Hamming distance: SKIPPED (would be slow)")
...
def benchmark_diversity():
    ...
    for n in test_sizes:
        ...
        if n <= 5000:
            start = time.time()
            stats = DiversityAnalyzer.pairwise_distance_stats(
                sequences, max_sample=None
            )
            elapsed = time.time() - start
            print(f"  Pairwise (full): {elapsed:.2f}s")
        else:
            print("  Pairwise (full): SKIPPED (O(n²) too slow)")
        ...
        # All metrics
        start = time.time()
        analyze_diversity(sequences, max_sample=5000)
        elapsed = time.time() - start
