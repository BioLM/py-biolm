...
class TestValidationError:
    ...
    def test_validation_error_creation(self):
        ...
        error = ValidationError(
            message="Test error", path="tasks[0].id", error_type="semantic"
        )
...
class TestProtocolValidate:
    ...
    def test_validate_missing_schema_version(self, missing_schema_version):
        ...
        assert result.is_valid is False
        assert any(
            "inputs" in e.message or "required" in e.message.lower()
    ...
    def test_validate_invalid_template_expression(self, invalid_template_expression):
        ...
        assert any(e.error_type in ("syntax", "semantic") for e in result.errors)
    ...
    def test_validate_nonexistent_file(self):
        ...
        assert len(result.errors) > 0
        assert any(
            "Failed" in e.message or "parse" in e.message.lower() for e in result.errors
        )
...
class TestCLIValidate:
    ...
    def test_cli_validate_json_output(self, valid_protocol_yaml):
        ...
        result = runner.invoke(
            cli, ["protocol", "validate", "--json", valid_protocol_yaml]
        )
    ...
    def test_cli_validate_json_output_invalid(self, missing_schema_version):
        ...
        runner = CliRunner()
        result = runner.invoke(
            cli, ["protocol", "validate", "--json", missing_schema_version]
        )
