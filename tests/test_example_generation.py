...
def test_get_sample_input():
    ...
    assert (
        generator._get_sample_input("pdb", "predict")
        == "ATOM      1  N   MET A   1      20.154  16.967  10.410  1.00 20.00           N"
    )
...
def test_generate_python_example(mock_model_schema_encode):
    ...
    example = generator._generate_python_example(
        "esm2-8m", "encode", input_type="sequence", schema=mock_model_schema_encode
    )
...
def test_generate_markdown_example():
    ...
    example = generator._generate_markdown_example(
        "esm2-8m", "encode", input_type="sequence"
    )
...
def test_get_example_function():
    ...
    with patch("biolmai.examples.ExampleGeneratorSync") as mock_generator_class:
        ...
        mock_generator.generate_example.assert_called_once_with(
            "esm2-8m", "encode", "python"
        )
...
def test_example_formats(mock_model_schema_encode):
    ...
    rst_example = generator._generate_rst_example("esm2-8m", "encode", "sequence")
    assert isinstance(rst_example, str)
    assert len(rst_example) > 0

    json_example = generator._generate_json_example("esm2-8m", "encode", "sequence")
...
def test_example_error_handling():
    ...
    example = generator._generate_python_example(
        "esm2-8m", "encode", input_type="sequence", schema=None
    )
