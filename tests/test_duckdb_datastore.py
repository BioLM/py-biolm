...
class TestDuckDBBasics:
    ...
    def test_deduplication_anti_join(self, duckdb_store):
        ...
        print(f"  ✓ Anti-join deduplication works correctly")
    ...
    def test_batch_predictions(self, duckdb_store):
        ...
        predictions = [
            {'sequence_id': seq_ids[0], 'prediction_type': 'tm', 
             'model_name': 'test', 'value': 50.0},
            {'sequence_id': seq_ids[1], 'prediction_type': 'tm', 
             'model_name': 'test', 'value': 60.0},
            {'sequence_id': seq_ids[2], 'prediction_type': 'tm', 
             'model_name': 'test', 'value': 70.0},
        ]
    ...
    def test_query_power(self, duckdb_store):
        ...
        # Add predictions
        predictions = [
            {'sequence_id': sid, 'prediction_type': 'score', 
             'model_name': 'test', 'value': float(i)}
...
class TestDuckDBPerformance:
    ...
    def test_large_batch_performance(self, duckdb_store):
        ...
        print(f"  ✓ Added 1000 sequences in {elapsed*1000:.0f}ms ({elapsed/1000*1000000:.0f}µs each)")
    ...
    def test_diff_mode_performance(self, duckdb_store):
        ...
        start = time.time()
        ids2 = duckdb_store.add_sequences_batch(batch2, deduplicate=True)
        elapsed = time.time() - start
        ...
        print(f"    Anti-join deduplication is vectorized!")
