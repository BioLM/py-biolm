...
def example_3_ranking_filter():
    ...
    # Top 10% by pLDDT
    filter3 = RankingFilter('plddt', method='percentile', percentile=10, ascending=False)
    df_top_percentile = filter3(df)
    
    print(f"\nTop 10% by pLDDT: {len(df_top_percentile)} sequences")
    print(f"pLDDT range: {df_top_percentile['plddt'].min():.1f} - {df_top_percentile['plddt'].max():.1f}")
...
def example_5_mlm_remasking():
    ...
    variants = remasker_conservative.generate_variants(
        parent_sequence,
        num_variants=5
    )
    
    for i, (var_seq, metadata) in enumerate(variants):
        print(f"  Variant {i+1}: {metadata['num_mutations']} mutations ({metadata['mutation_rate']:.1%})")
    
    # Moderate remasking (15% masked)
    print("\nModerate remasking (15% masked, medium temp):")
    remasker_moderate = MLMRemasker(MODERATE_CONFIG)
    
    variants = remasker_moderate.generate_variants(
        parent_sequence,
        num_variants=5
    )
    ...
    variants = remasker_custom.generate_variants(
        parent_sequence,
        num_variants=3
    )
